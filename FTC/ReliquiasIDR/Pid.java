...
public class Pid implements Runnable{
    ...
    public void run(){
        ...
        while(!kyss){
            iteration++;
        if(iteration == 10){
          t.addLine(Integer.toString(iteration));
          t.update();
        }
        double k = System.nanoTime();
            double trueAngle = 0;//Math.toRadians(d.getAngles()[0]);
            ...
            for(int i = 0; i < 3; i++){
              ...
              if(!turbo)
                ...
                motor_proportions[i] = lengthOf(vec) * motor_proportions[i] + angleDelta;
            }
            ...
            prev = cur;
         /*t.addLine(Double.toString(System.nanoTime() - k));
         t.update();*/
         }
    }
