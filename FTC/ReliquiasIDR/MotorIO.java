...
public class MotorIO implements Runnable{
    ...
    public void run(){
        ...
        CRServo pidennin = (CRServo) hwmap.get("kasi");
        ...
        while(!kyss){
            ...
            t.addLine(Double.toString(clamp(Math.abs(e1), 0.07, 0.8) * Math.signum(e1)));
            t.addLine(Double.toString(clamp(Math.abs(e2), 0.07, 0.8) * Math.signum(e2)));
            t.addLine(Double.toString(clamp(Math.abs(e3), 0.07, 0.8) * Math.signum(e3)));
            t.update();
